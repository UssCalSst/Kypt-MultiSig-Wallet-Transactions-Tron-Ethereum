...
if (!readyA)
{
    Console.WriteLine("Alice failed to get ready");
    return;
}
...
if (!readyB)
{
    Console.WriteLine("Bob failed to get ready");
    return;
}
...
if (!canStart)
{
    Console.WriteLine("Something went wrong during group signing!");
    return;
}
...
if (!isFinished)
{
    Console.WriteLine("Failed to finalize the group");
}

// The current raw (unsigned) transaction
string rawTransactionHex = testGroup.CurrentTransaction.ToHex();
